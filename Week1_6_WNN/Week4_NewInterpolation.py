...
def process_gait_cycles(insole_data, force_data, strikes, offs):
    """
    ...
    num_points = 100
    norm_insole = np.zeros((num_points, insole_data.shape[1], num_valid))
    norm_force = np.zeros((num_points, force_data.shape[1], num_valid))
...
norm_right_insole_pp, avg_right_insole_pp, std_right_insole_pp, _, _, _ = process_gait_cycles(
    ...
    gait_trackers.force_plate_ds_r[:, 2:3],  # Force plate vertical GRF（Z方向）可以作为reference
    gait_insole.strike_r,
    gait_insole.off_r)
...
norm_left_insole_pp, avg_left_insole_pp, std_left_insole_pp, _, _, _ = process_gait_cycles(
    ...
    gait_trackers.force_plate_ds_l[:, 2:3],
    gait_insole.strike_l,
    gait_insole.off_l)
...
norm_right_insole, avg_right_insole, std_right_insole, _, _, _ = process_gait_cycles(
    ...
    gait_trackers.force_plate_ds_r[:, 2:3],
    gait_insole.strike_r,
    gait_insole.off_r)
