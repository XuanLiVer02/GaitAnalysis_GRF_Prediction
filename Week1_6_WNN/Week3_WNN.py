...
from Week3_PCAMI import X_reduced
import numpy as np
import torch
...
from Week3_LoadData import grf_slow, grm_slow

######AFTER PCA#######
norm_right_insole, avg_right_insole, std_right_insole = process_gait_cycles(
    X_reduced, gait_insole.strike_r, gait_insole.off_r)
print('norm_right_insole', norm_right_insole)
print(norm_right_insole.shape)      #100,50,103
...
Test = np.zeros((grf_slow.shape[2], grf_slow.shape[0], 6))
...
# Y: [100, 100, 3]
print(X)
for i in range(grf_slow.shape[2]):
    Test[i] = np.concatenate((grf_slow[:,:,i], grm_slow[:,:,i]), axis=1)
...
n_hidden = 21  # Number of hidden neurons (adjust as needed)
