...
from Week4_AnotherPCA import *
from Week4_LoadDataSlow import *
from Week4_LoadDataFast import *
import numpy as np
import torch
...
steps_fast = 119
##Normal & Slow##
#norm_right_cop_insole = np.concatenate([norm_right_cop_insole[:, :, :steps], norm_right_cop_insole_slow[:, :, :steps_slow]], axis=2)
#norm_right_insole = np.concatenate([norm_right_insole[:, :, :steps], norm_right_insole_slow[:, :, :steps_slow], norm_right_cop_insole_fast[:, :, :steps_fast]], axis=2)
#norm_right_insole_pp = np.concatenate([norm_right_insole_pp[:, :, :steps], norm_right_insole_pp_slow[:, :, :steps_slow]], axis=2)
#norm_right_fti = np.concatenate([norm_right_fti[:, :, :steps], norm_right_fti_slow[:, :, :steps_slow]], axis=2)
#norm_right_ft = np.concatenate([norm_right_ft[:, :, :steps], norm_right_ft_slow[:, :, :steps_slow]], axis=2)
#norm_right_contact = np.concatenate([norm_right_contact[:, :, :steps], norm_right_contact_slow[:, :, :steps_slow]], axis=2)
#norm_right_grf = np.concatenate([norm_right_grf[:, :, :steps], norm_right_grf_slow[:, :, :steps_slow]], axis=2)
#norm_right_grm = np.concatenate([norm_right_grm[:, :, :steps], norm_right_grm_slow[:, :, :steps_slow]], axis=2)

norm_right_cop_insole = np.concatenate([norm_right_cop_insole[:, :, :steps], norm_right_cop_insole_slow[:, :, :steps_slow], norm_right_cop_insole_fast[:, :, :steps_fast]], axis=2)
...
norm_right_insole_pp = np.concatenate([norm_right_insole_pp[:, :, :steps], norm_right_insole_pp_slow[:, :, :steps_slow], norm_right_insole_pp_fast[:, :, :steps_fast]], axis=2)
norm_right_fti = np.concatenate([norm_right_fti[:, :, :steps], norm_right_fti_slow[:, :, :steps_slow], norm_right_fti_fast[:, :, :steps_fast]], axis=2)
norm_right_ft = np.concatenate([norm_right_ft[:, :, :steps], norm_right_ft_slow[:, :, :steps_slow], norm_right_ft_fast[:, :, :steps_fast]], axis=2)
norm_right_contact = np.concatenate([norm_right_contact[:, :, :steps], norm_right_contact_slow[:, :, :steps_slow], norm_right_contact_fast[:, :, :steps_fast]], axis=2)
norm_right_grf = np.concatenate([norm_right_grf[:, :, :steps], norm_right_grf_slow[:, :, :steps_slow], norm_right_grf_fast[:, :, :steps_fast]], axis=2)
norm_right_grm = np.concatenate([norm_right_grm[:, :, :steps], norm_right_grm_slow[:, :, :steps_slow], norm_right_grm_fast[:, :, :steps_fast]], axis=2)
...
X = np.zeros((num_cycles, time_steps, 6))
...
for i in range(num_cycles):
    ...
    # Input: insole_pp + cop
    X[i] = np.concatenate((insole_pp, cop, fti, ft, cont), axis=1)
    #X[i] = insole_r
