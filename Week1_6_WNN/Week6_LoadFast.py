...
# Load data
data = loadmat('C:/Users/XuanL/Documents/MATLAB/Gait Analysis/ABLE1_112024_1.5.mat', struct_as_record=False,
               squeeze_me=True)
...
#        insole_stance = X_splits
    ...
    for i in range(num_valid):  # Every step
        ...
        # 根据时间对齐，找出 t_force 中不超过 t_insole[-1] 的部分
        valid_force_idx = t_force_full <= t_insole[-1]
        t_force = t_force_full[valid_force_idx]
        force_cycle = force_cycle[valid_force_idx, :]  # 截断 force_cycle
...
# Right foot GRF
_, _, _, norm_right_grf_fast, avg_right_grf_fast, std_right_grf_fast = process_gait_cycles(
...
# Left foot GRF
_, _, _, norm_left_grf_fast, avg_left_grf_fast, std_left_grf_fast = process_gait_cycles(
...
# Right foot GRM
_, _, _, norm_right_grm_fast, avg_right_grm_fast, std_right_grm_fast = process_gait_cycles(
    gait_insole.pp_r.reshape(-1, 1),
    grm_r,
    gait_insole.strike_r,
    gait_insole.off_r,
...
# Left foot GRM
_, _, _, norm_left_grm_fast, avg_left_grm_fast, std_left_grm_fast = process_gait_cycles(
    gait_insole.pp_l.reshape(-1, 1),
    grm_l,
    gait_insole.strike_l,
    gait_insole.off_l,
...
# Right foot COP
_, _, _, norm_right_cop_fp_fast, avg_right_cop_fp_fast, std_right_cop_fp_fast = process_gait_cycles(
    gait_insole.pp_r.reshape(-1, 1),
...
# Left foot COP
_, _, _, norm_left_cop_fp_fast, avg_left_cop_fp_fast, std_left_cop_fp_fast = process_gait_cycles(
    gait_insole.pp_l.reshape(-1, 1),
...
norm_right_pp_pos_fast, avg_right_pp_pos_fast, std_right_pp_pos_fast, _, _, _ = process_gait_cycles(
    right_pp_pos,
    grf_r,
    gait_insole.strike_r,
    gait_insole.off_r,
...
norm_left_pp_pos_fast, avg_left_pp_pos_fast, std_left_pp_pos_fast, _, _, _ = process_gait_cycles(
    left_pp_pos,
    grf_l,
    gait_insole.strike_l,
    gait_insole.off_l,
...
# Used in CNN
norm_right_insole_fast_all, avg_right_insole_fast_all, std_right_insole_fast_all, _, _, _ = process_gait_cycles(
    gait_insole.insole_r,
    grf_r,
    gait_insole.strike_r,
    gait_insole.off_r,
    gait_trackers.strike_r,
    gait_trackers.off_r,)
norm_left_insole_fast_all, avg_left_insole_fast_all, std_v_insole_fast_all, _, _, _ = process_gait_cycles(
    gait_insole.insole_l,
    grf_l,
    gait_insole.strike_l,
    gait_insole.off_l,
...
def plot_graph():
    ...
    plt.subplot(3, 2, 1)
    plt.plot(x_axis, avg_right_grf_fast[:, 0], 'r', linewidth=2)
    plt.fill_between(x_axis, avg_right_grf_fast[:, 0] + std_right_grf_fast[:, 0],
                     avg_right_grf_fast[:, 0] - std_right_grf_fast[:, 0], color='r', alpha=0.2)
    ...
    plt.subplot(3, 2, 3)
    plt.plot(x_axis, avg_right_grf_fast[:, 1], 'r', linewidth=2)
    plt.fill_between(x_axis, avg_right_grf_fast[:, 1] + std_right_grf_fast[:, 1],
                     avg_right_grf_fast[:, 1] - std_right_grf_fast[:, 1], color='r', alpha=0.2)
    ...
    plt.subplot(3, 2, 5)
    plt.plot(x_axis, avg_right_grf_fast[:, 2], 'r', linewidth=2)
    plt.fill_between(x_axis, avg_right_grf_fast[:, 2] + std_right_grf_fast[:, 2],
                     avg_right_grf_fast[:, 2] - std_right_grf_fast[:, 2], color='r', alpha=0.2)
