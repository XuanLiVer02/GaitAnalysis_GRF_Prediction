...
from Week3_PCA_Left import X_reduced
import numpy as np
import torch
...
######AFTER PCA#######
norm_left_insole, avg_left_insole, std_left_insole = process_gait_cycles(
    X_reduced, gait_insole.strike_l, gait_insole.off_l)
print(norm_left_insole.shape)      #100,50,103

norm_left_grf = norm_left_grf[:, :, :100]
norm_left_cop_insole = norm_left_cop_insole[:, :, :100]
norm_left_insole_pp = norm_left_insole_pp[:, :, :100]
norm_left_insole = norm_left_insole[:, :, :100]
...
X = np.zeros((num_cycles, time_steps, 8))
Y = np.zeros((num_cycles, time_steps, 3))

for i in range(num_cycles):
    insole_l = norm_left_insole[:, :, i]
    # shape: (100, 1)
    insole_pp = norm_left_insole_pp[:, :, i]
    # shape: (100, 2)
    cop = norm_left_cop_insole[:, :, i]
    # shape: (100, 3)
    grf = norm_left_grf[:, :, i]
    # Input: insole_pp + cop
    X[i] = np.concatenate((insole_l, insole_pp, cop), axis=1)
    Y[i] = grf
# X: [100, 100, 8]
# Y: [100, 100, 3]
...
n_hidden = 17  # Number of hidden neurons (adjust as needed)
