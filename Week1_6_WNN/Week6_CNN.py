...
X_train, _, _ = normalize_data_seq(X_train)
X_val, _, _ = normalize_data_seq(X_val)
X_test, _, _ = normalize_data_seq(X_test)
y_train, Y_min, Y_max = normalize_data_seq(y_train)
y_val, _, _ = normalize_data_seq(y_val)
y_test, _, _ = normalize_data_seq(y_test)
Y_min = np.transpose(Y_min, (0, 2, 1))
Y_max = np.transpose(Y_max, (0, 2, 1))
...
train(model, train_loader, val_loader, criterion, optimizer, epochs=1000)
...
y_pred = np.transpose(y_pred, (0, 2, 1))  # shape: [20, 6, 100]
y_test = np.transpose(y_test, (0, 2, 1))  # shape: [20, 6, 100]
...
y_pred_denorm = denormalize_data_seq(y_pred, Y_min, Y_max)   # shape: [20, 100, 6], [1,1,100]
