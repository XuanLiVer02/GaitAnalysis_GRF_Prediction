...
teacher_model = VisionTransformer(output_features=1, num_layers=6, time_steps=101).to('cpu')
teacher_model.load_state_dict(torch.load('early_stop_ViT.pth'))

def get_cls_feature(model, x):
    x = model.patch_embedding(x)
    x = model.positional_encoding(x)
    encoder_output = model.transformer_encoder(x)
...
def train(model, teacher_model, train_loader, val_loader, criterion, optimizer, epochs):
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            loss_teacher = criterion(output, y_batch)

            student_features = get_cls_feature(model, X_batch)

            with torch.no_grad():
                teacher_model.to(device)
                teacher_features = get_cls_feature(teacher_model, X_batch)
                teacher_model.to('cpu')

            loss_distill = criterion(teacher_features, student_features)
            loss = alpha * loss_teacher + (1 - alpha) * loss_distill
            loss.backward()
            optimizer.step()
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_pool)):
    ...
    model = VisionTransformer(output_features=6, num_layers=2, time_steps=100).to(device)
    ...
    for i in range(2):
        for param_name in param_names:
            key_teacher = f"transformer_encoder.layers.{i}.{param_name}"
            key_student = f"transformer_encoder.layers.{i}.{param_name}"
            if key_teacher in teacher_state_dict:
                student_state_dict[key_student] = teacher_state_dict[key_teacher]

    model.patch_embedding.load_state_dict(teacher_model.patch_embedding.state_dict())
    model.positional_encoding.load_state_dict(teacher_model.positional_encoding.state_dict())
    model.load_state_dict(student_state_dict)
    #
    # param_groups = [
    #     {"params": model.fc_out.parameters(), "lr": 1e-6}
    # ]
    ...
    train_loader = DataLoader(TensorDataset(X_train_fold_t, y_train_fold_t), batch_size=50, shuffle=True)
    val_loader = DataLoader(TensorDataset(X_val_fold_t, y_val_fold_t), batch_size=50)
    train(model, teacher_model, train_loader, val_loader, criterion, optimizer, epochs=epochs_model)
