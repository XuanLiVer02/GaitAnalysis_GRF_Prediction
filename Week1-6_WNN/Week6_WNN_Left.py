...
steps = 100
norm_left_cop_insole = norm_left_cop_insole[:, :, :steps]
norm_left_insole = norm_left_insole[:, :, :steps]
norm_left_insole_pp = norm_left_insole_pp[:, :, :steps]
norm_left_fti = norm_left_fti[:, :, :steps]
norm_left_ft = norm_left_ft[:, :, :steps]
norm_left_contact = norm_left_contact[:, :, :steps]
norm_left_pp_pos = norm_left_pp_pos[:, :, :steps]
print('norm_left_insole_pp:', norm_left_insole_pp.shape)   #(100, 1, 100)
print(norm_left_cop_insole.shape)  #(100, 2, 100)
print(norm_left_grf.shape)         #(100, 3, 100)
print(norm_left_insole_pp.shape)   #(100, 1, 100)
print(norm_left_grf.shape)
#####
# num_cycles: how many steps (100)
# time_steps: how many time point each step (100)
#####
num_cycles = norm_left_cop_insole.shape[2]  # 100
time_steps = norm_left_cop_insole.shape[0]  # 100
print(num_cycles), print(time_steps)
# Input & Output Empty arrays
X = np.zeros((num_cycles, time_steps, 10))
Y = np.zeros((num_cycles, time_steps, 6))

for i in range(num_cycles):
    insole_l = norm_left_insole[:, :, i]
    insole_pp = norm_left_insole_pp[:, :, i]   # shape: (100, 1)
    cop = norm_left_cop_insole[:, :, i]    # shape: (100, 2)
    fti = norm_left_fti[:, :, i]   # shape: (100, 1)
    #ft = norm_left_ft[:, :, i]   # shape: (100, 1)
    cont = norm_left_contact[:, :, i]   # shape: (100, 1)
    pp_pos = norm_left_pp_pos[:, :, i]   # shape: (100, 1)
    # shape: (100, 3)
    grf = norm_left_grf[:, :, i]
    grm = norm_left_grm[:, :, i]
    # Input: insole_pp + cop
    X[i] = np.concatenate((insole_l, cop, cont, fti, pp_pos), axis=1)
    #X[i] = insole_l
...
X_train, X_min, X_max = normalize_data_seq(X_train)
X_val= normalize_data_seq_with_given_min_max(X_val, X_min, X_max)
...
# Step 2: 定义一个函数计算相关系数、RMSE、NRMSE
