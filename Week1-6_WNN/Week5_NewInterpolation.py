...
#        insole_stance = X_splits
    ...
    for i in range(num_valid):  # Every step
        ...
        #standard_time = np.linspace(0, t_insole[-1], num_points)
        standard_time = np.unique(np.concatenate((t_insole, t_force))) # eg. 105
        standard_time.sort()

        if len(standard_time) > 100:
            # Downsample to 100 points
            indices = np.linspace(0, len(standard_time) - 1, num=100, dtype=int)
            standard_time = standard_time[indices]
        elif len(standard_time) < 100:
            dt = standard_time[-1] - standard_time[-2]
            padding = standard_time[-1] + dt * np.arange(1, 100 - len(standard_time) + 1)
            standard_time = np.concatenate([standard_time, padding])
        ...
        for j in range(insole_cycle.shape[1]):
            ...
            raw_insole[:, j, i] = f_insole(standard_time)

        # Force
        for j in range(force_cycle.shape[1]):
            f_force = interpolate.interp1d(t_force, force_cycle[:, j], kind='nearest', fill_value='extrapolate')
            raw_force[:, j, i] = f_force(standard_time)
...
norm_right_pp_pos, avg_right_pp_pos, std_right_pp_pos, _, _, _ = process_gait_cycles(
    right_pp_pos,
    grf_r,
    gait_insole.strike_r,
    gait_insole.off_r,
...
norm_left_pp_pos, avg_left_pp_pos, std_left_pp_pos, _, _, _ = process_gait_cycles(
    left_pp_pos,
    grf_l,
    gait_insole.strike_l,
    gait_insole.off_l,
